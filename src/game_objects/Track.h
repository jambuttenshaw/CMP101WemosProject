...
private:
    ...
    Point m_TrackVertices[3];//89]; // an array of points that define the shape of the track
    unsigned int m_TrackEdgeIndices[6];//178]; // an array of indices that define how the points that make up the track should be joined together
    unsigned int m_TrackAreaIndices[3];//267]; // an array of indices that define the area that the track covers, for checking when the car is on or off the track
